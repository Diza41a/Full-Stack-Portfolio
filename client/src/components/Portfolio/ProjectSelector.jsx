...
export default function ProjectSelector() {
  const [currentProject, setProject] = useState('Atelier Clothing Platform');
  ...
  // Rendering
  const renderProjects = (arr, disabled) => (
      ...
      <ul className={disabled ? 'disabled' : ''} onClick={!disabled ? switchProject : null}>
        {arr.map((item, i) => (
          <li
            ...
            className={!disabled && item.title === currentProject ? 'selected' : null}
          >
            {item.title}
  ...
  return (
    <Portfolio.Selector className="project-selector">
      <div className="project-selector-list">
        ...
        {renderProjects(inProgress, true)}
      </div>
